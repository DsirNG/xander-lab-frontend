...
- **下拉浮层硬性规定**：所有弹出层（`CustomSelect`、`CreatableMultiSelect` 等）默认禁止"点击空白收起"，只能通过选择选项、Esc 或所在弹窗右上角关闭按钮关闭；业务方如确需点击外部关闭，必须显式传 `closeOnOutsideClick` 并自行向组件注释里登记原因。下拉浮层必须使用 Portal 渲染（脱离弹窗滚动容器与层级），不得被弹窗标题/遮罩覆盖。
