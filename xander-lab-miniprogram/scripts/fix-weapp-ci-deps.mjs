...
function fixCompatData() {
  const cdDir = path.join(pnpmDir, '@babel+compat-data@8.0.0', 'node_modules', '@babel', 'compat-data');
...
function fixPolyfillCorejs3() {
  ...
  const target = findPkgDir('babel-plugin-polyfill-corej_', 'babel-plugin-polyfill-corejs3', (v) => v.startsWith('0.6'));
  if (!target) {
...
function fixLruCache() {
  ...
  if (!cur || cur.version !== '11.5.2') {
    ...
    if (fs.existsSync(hoisted)) {
      fs.renameSync(hoisted, path.join(hoistedDir, `lru-cache.bak-${(cur && cur.version) || 'old'}`));
    }
