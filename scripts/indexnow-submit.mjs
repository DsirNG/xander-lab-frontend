...
async function readUrls() {
  ...
  return [...new Set([...xml.matchAll(/<loc>\s*([^<]+?)\s*<\/loc>/gi)].map((m) => m[1]))]
...
async function main() {
  ...
  if (!key || !urls.length) throw new Error('IndexNow key 或 sitemap URL 为空');
  console.log(`IndexNow: ${urls.length} 个 URL，${dryRun ? 'dry-run' : '准备提交'}`);
  if (dryRun) return;
  ...
  for (let offset = 0; offset < urls.length; offset += BATCH_SIZE) {
    ...
    const response = await axios.post(ENDPOINT, {
      ...
      urlList,
    }, { timeout: 15_000, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
    console.log(`已提交 ${offset + urlList.length}/${urls.length}，HTTP ${response.status}`);
  }
}

main().catch((error) => {
  const status = error.response?.status;
  console.error(`IndexNow 提交失败${status ? ` (HTTP ${status})` : ''}: ${error.message}`);
  process.exitCode = 1;
});
