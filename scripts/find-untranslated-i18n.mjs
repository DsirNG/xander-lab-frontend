...
async function main() {
    const enModule = await import(
        pathToFileURL(path.join(LOCALES_DIR, "en.js")).href
    );
    const enFlat = flatten(enModule.default);

    for (const locale of locales) {
        const locModule = await import(
            pathToFileURL(path.join(LOCALES_DIR, `${locale}.js`)).href
        );
        ...
        for (const [key, enVal] of Object.entries(enFlat)) {
            ...
            if (skipPatterns.some((p) => p.test(key) || p.test(String(enVal))))
                continue;
