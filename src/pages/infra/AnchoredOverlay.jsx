...
const PhaseCard = ({ phase, index }) => {
    return (
        ...
        >
            <div className="absolute left-[-16px] top-0 w-8 h-8 rounded-full bg-white dark:bg-slate-900 border-2 border-primary flex items-center justify-center text-primary shadow-sm">
                {index === 0 && <Compass className="w-4 h-4" />}
                {index === 1 && <Cpu className="w-4 h-4" />}
                {index === 2 && <Box className="w-4 h-4" />}
            </div>

            <div className="bg-white dark:bg-slate-900/50 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 hover:border-primary/50 transition-colors shadow-sm">
