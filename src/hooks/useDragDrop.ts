...
export function useDragDrop<T = any>({
    ...
    const dragMoveHandlerRef = useRef<((ev: PointerEvent) => void) | null>(
        null,
    );
            ...
            if (getDragPreview) {
                ...
                if (res) {
                    ...
                    const onMove = (ev: PointerEvent) => {
                        if (!dragPreviewElRef.current || !isDraggingRef.current)
                            return;

                        if (previewCtrlRef.current?.updatePosition) {
                            previewCtrlRef.current.updatePosition(
                                ev.clientX,
                                ev.clientY,
                            );
                        } else {
    ...
    const handleDragOver = useCallback(
        (item: T, e: React.PointerEvent) => {
            ...
            // 允许时显示并更新文案
            const text = getDropHintText
                ? getDropHintText(draggedItemRef.current, item)
                : "";
            if (text) {
