...
describe("knowledgeService", () => {
    it("测验成绩单和录音记录是两条独立的历史，端点不能串", () => {
        ...
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/recitations/materials/9/quizzes",
            undefined,
            undefined,
        );
        knowledgeService.listAttempts(9);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/recitations/materials/9/attempts",
            undefined,
            undefined,
        );
    });
    ...
    it("materials 的增删查与上传端点映射", () => {
        ...
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/recitations/materials",
            undefined,
            undefined,
        );
        knowledgeService.get(9);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/recitations/materials/9",
            undefined,
            undefined,
        );
        knowledgeService.getAttempt(5);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/recitations/attempts/5",
            undefined,
            undefined,
        );
        ...
        expect(apiMock.upload).toHaveBeenCalledWith(
            ...
            {
                fieldName: "file",
                extraData: { clientRequestId: "request-123" },
                config: { signal: 1 },
            },
        );
    });
    ...
    it("归档和恢复是同一个端点，靠 archived 参数区分", () => {
        ...
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/recitations/materials/9/archive",
            null,
