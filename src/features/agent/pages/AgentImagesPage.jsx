...
const EXAMPLES = [
    {
        id: 1,
        url: "https://images.unsplash.com/photo-1578632767115-351597cf2477?w=300&q=80",
        title: "创建女孩漫画",
    },
    {
        id: 2,
        url: "https://images.unsplash.com/photo-1544502062-f82887f03d1c?w=300&q=80",
        title: "动漫男孩",
    },
    {
        id: 3,
    ...
    {
        id: 4,
        url: "https://images.unsplash.com/photo-1490818387583-1b5f22223403?w=300&q=80",
        title: "Summer list",
    },
    {
        id: 5,
        url: "https://images.unsplash.com/photo-1513519245088-0e12902e5a38?w=300&q=80",
        title: "杂货合集",
    },
];
...
const AgentImagesPage = ({ onGenerate }) => {
        ...
        <div className="flex h-full w-full flex-col bg-surface overflow-y-auto">
            <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <h1 className="mb-8 text-3xl font-bold text-ink">
                    {t("blog.agentImages.title")}
                </h1>

                {/* Input Bar */}
                <form
                    onSubmit={handleSubmit}
                    className="relative mb-12 flex items-center w-full max-w-3xl mx-auto rounded-full border border-border bg-canvas px-4 py-3 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all"
                >
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            type="button"
                            className="flex items-center gap-1.5 px-2 text-sm font-semibold text-ink-muted hover:text-ink transition"
                        >
                        ...
                        <button
                            type="button"
                            className="grid h-8 w-8 place-items-center rounded-full text-ink-muted hover:bg-surface-muted transition"
                        >
                ...
                <div className="mb-12">
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 snap-x custom-scrollbar">
                        {EXAMPLES.map((ex) => (
                            <div
                                key={ex.id}
                                className="group relative h-64 w-44 shrink-0 snap-center overflow-hidden rounded-[2rem] cursor-pointer shadow-sm border border-border/50 hover:shadow-md transition-shadow"
                            >
                                <img
                                    src={ex.url}
                                    alt={ex.title}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-border/50 bg-canvas py-16 text-center">
                            <p className="text-sm text-ink-muted">
                                {t("blog.agentImages.emptyTitle")}
                            </p>
                            <p className="text-xs text-ink-muted/70">
                                {t("blog.agentImages.emptyHint")}
                            </p>
                        </div>
                    ) : (
                        <>
                            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                                {images.map((item) => (
                                    <div
                                        key={item.id}
                                        className="group relative aspect-square w-full overflow-hidden rounded-2xl cursor-pointer bg-surface-muted shadow-sm border border-border/50 hover:shadow-md transition-shadow"
                                    >
                                        <img
                                            src={item.url}
                                            alt={item.title || ""}
                            ...
                            {hasMore && (
                                <div className="mt-6 flex justify-center">
                                    ...
                                    >
                                        ...
                                        {loadingMore
                                            ? t("blog.agentImages.loadingMore")
                                            : t("blog.agentImages.loadMore")}
                                    </button>
                                </div>
                            )}
                        </>
