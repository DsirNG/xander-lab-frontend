...
const AgentSharedView = () => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        get(`/api/agent/conversations/shared/${shareToken}`, {
            _silent: true,
            ...
            .catch((err) => {
                if (controller.signal.aborted || err?.code === "ERR_CANCELED")
                    return;
                setError(
                    err.message || t("blog.agentChat.loadFailed", "加载失败"),
                );
            })
            .finally(() => {
                    ...
                    {messages.map((message) => {
                        ...
                        if (message.kind === "thought") {
                            return (
                                <div
                                    key={message.id}
                                    className="flex items-start gap-2 rounded-xl border border-border bg-canvas px-3 py-2 text-xs leading-5 text-ink-muted"
                                >
