import { BookOpen, FileText, Globe, Image as ImageIcon, Loader2, Mic, Paperclip, PenLine, Send, Square, X } from 'lucide-react';
...
const QUICK_ACTIONS = [
  { key: 'generateImage', icon: ImageIcon, iconClassName: 'bg-emerald-50 text-emerald-500' },
  { key: 'searchWeb', icon: Globe, iconClassName: 'bg-orange-50 text-orange-500' },
  { key: 'generatePractice', icon: PenLine, iconClassName: 'bg-blue-50 text-blue-500' },
  { key: 'importKnowledge', icon: BookOpen, iconClassName: 'bg-purple-50 text-purple-500' },
];
      ...
      {attachments.length > 0 ? (
        <div className="flex flex-wrap gap-2 px-2 pb-2">
          {attachments.map((attachment) => (
            <AttachmentPreview key={attachment.url} attachment={attachment} onRemove={onRemoveAttachment} />
          ))}
        </div>
      ) : null}
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-2.5">
        {QUICK_ACTIONS.map(({ key, icon: Icon, iconClassName }) => (
          <button
            key={key}
            type="button"
            onClick={() => onInputChange(t(`workspace.agent.actions.${key}`))}
          ...
          >
            <span className={`grid h-5 w-5 place-items-center rounded-md ${iconClassName}`}>
