    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    {running ? (
                        <Loader2 className="h-3.5 w-3.5 animate-spin text-[#9ea3b9]" />
                    ) : null}
                    {unread ? (
                ...
                <span
                    className={`truncate text-sm font-normal ${active ? "text-[#6055f6]" : "text-[#0d0d0d]"}`}
                >
            ...
            <div className="flex shrink-0 items-center pr-2.5 pl-2">
                {formattedTime ? (
                    <span className="text-xs text-[#9ea3b9] group-hover:hidden">
                        {formattedTime}
                    </span>
                ) : null}
                <button
            ...
            <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3 rounded-xl border border-[#ececf4] px-3.5 py-2.5">
                    ...
                    <input
                        ...
                        placeholder={t(
                            "blog.agentChat.searchPlaceholder",
                            "搜索会话...",
                        )}
                        autoFocus
                ...
                <div className="custom-scrollbar max-h-[50vh] space-y-1 overflow-y-auto pr-1">
                    ...
                    {filteredSessions.length > 0 ? (
                        ...
                        <div className="py-8 text-center text-xs text-[#8e94aa]">
                            {query
                                ? t(
                                      "blog.agentChat.searchNoMatches",
                                      "没有找到匹配的会话",
                                  )
                                : t(
                                      "blog.agentChat.noSessions",
                                      "暂无会话记录",
                                  )}
                        </div>
                    )}
...
const WorkspaceAgentSidebar = ({
    ...
    const { pinnedSessions, recentSessions } = useMemo(() => {
        ...
        return {
            pinnedSessions: sessions
                .filter((session) => Number(session.isPinned) === 1)
                .sort(byTimeDesc),
    ...
    const visiblePinnedSessions = expandedPinned
        ? pinnedSessions
        : pinnedSessions.slice(0, 3);
    const hiddenPinnedCount =
        pinnedSessions.length - visiblePinnedSessions.length;
                ...
                <aside className="absolute inset-y-0 left-0 z-20 flex w-[250px] flex-col overflow-hidden border-r border-[#ececf4] bg-[#fcfcfd] p-4 shadow-[8px_0_24px_rgba(17,20,38,0.06)]">
                    <div className="flex h-10 shrink-0 items-center">
                        ...
                        <div className="ml-2 flex min-w-0 flex-1 items-center">
                            ...
                            <button
                                ...
                                className="grid h-9 w-9 shrink-0 place-items-center rounded-xl text-[#6c7293] transition-colors hover:bg-[#f7f6fc]"
                                title={t(
                                    "workspace.agent.collapseDrawer",
                                    "收起对话框",
                                )}
                            >
                    ...
                    <div className="mt-4 flex h-9 shrink-0 items-center">
                        <button
                            ...
                            title={t(
                                "workspace.agent.newConversation",
                                "新建对话",
                            )}
                        >
