...
const McpAuthorizationPanel = () => {
    ...
    const endpoints = [
        {
            id: "blog",
            name: t("profile.mcp.blogEndpoint"),
            url: `${baseUrl}/api/mcp`,
        },
        {
            id: "csdn",
            name: t("profile.mcp.csdnEndpoint"),
            url: `${baseUrl}/api/mcp/csdn`,
        },
        {
            id: "juejin",
            name: t("profile.mcp.juejinEndpoint"),
            url: `${baseUrl}/api/mcp/juejin`,
        },
        {
            id: "dual",
            name: t("profile.mcp.dualEndpoint"),
            url: `${baseUrl}/api/mcp/dual`,
        },
    ];
    ...
    useEffect(() => {
        ...
        mcpOAuthService
            ...
            .then((request) => {
                ...
                setSelectedScopes(
                    preselectScopes(request.scopes, request.previouslyGranted),
                );
            })
            .catch(
                (error) =>
                    active &&
                    toast.error(
                        error?.message || t("profile.mcp.consentLoadFailed"),
                    ),
            );
        return () => {
            active = false;
    ...
    useEffect(() => {
        ...
        mcpOAuthService
            ...
            .then(
                (items) =>
                    active && setClients(Array.isArray(items) ? items : []),
            )
            .catch(() => {
                /* The profile remains usable when no OAuth table is deployed yet. */
            });
        return () => {
            active = false;
        };
    ...
    const preselectScopes = (requested, previouslyGranted) => {
        ...
        const inherited = prior.length
            ? requestedScopes.filter((scope) => prior.includes(scope))
            : [];
    ...
    const completeOAuthRequest = async (approve) => {
        if (!oauthRequest || oauthBusy || (approve && !selectedScopes.length))
            return;
        setOauthBusy(true);
        try {
            const result = approve
                ? await mcpOAuthService.approveAuthorization(
                      oauthRequest.requestId,
                      selectedScopes,
                  )
                : await mcpOAuthService.denyAuthorization(
                      oauthRequest.requestId,
                  );
            ...
            <div className="mx-auto w-full max-w-3xl">
                <div className="mb-6 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                        <div className="mb-1 text-micro font-bold uppercase tracking-[0.2em] text-accent">
                            MCP
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="text-body font-bold text-ink">
                                    {t("profile.mcp.consentTitle")}
                                </div>
                                <div className="mt-1 text-caption text-ink-muted">
                                    {t("profile.mcp.consentDescription", {
                                        client: oauthRequest.clientName,
                                    })}
                                </div>
                                ...
                                <div className="mt-2 text-micro text-ink-muted">
                                    {t("profile.mcp.consentScopeHint")}
                                </div>
                                <div className="mt-4 flex flex-wrap gap-2">
                                    <button
                                        type="button"
                                        onClick={() =>
                                            completeOAuthRequest(true)
                                        }
                                        disabled={
                                            oauthBusy || !selectedScopes.length
                                        }
                    ...
                    <div className="mt-3 space-y-2">
                        {endpoints.map((endpoint) => {
                            ...
                            return (
                                ...
                                >
                                    <span className="shrink-0 text-caption font-bold text-ink-secondary">
                                        {endpoint.name}
                                    </span>
                                    <code
                                        className="min-w-0 flex-1 truncate font-mono text-caption text-ink-muted"
                                        title={endpoint.url}
                                    >
                                        {endpoint.url}
                                    </code>
                            ...
                            {clients.map((client) => (
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        onClick={() =>
                                            revokeClient(client.clientId)
                                        }
                                        disabled={
                                            revokingClient === client.clientId
                                        }
                                        ...
                                        aria-label={t(
                                            "profile.mcp.revokeClient",
                                        )}
                                        title={t("profile.mcp.revokeClient")}
                                    >
                                        {revokingClient === client.clientId ? (
                                            <Unplug className="h-4 w-4 animate-pulse" />
                                        ) : (
                                            <X className="h-4 w-4" />
                                        )}
                                    </button>
