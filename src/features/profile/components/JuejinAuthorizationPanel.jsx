...
const JuejinAuthorizationPanel = () => {
        ...
        try {
            ...
            if (status?.status === "PENDING") {
                ...
                if (
                    ...
                    status.qrCodeDataUrl !== qrCodeRef.current
                ) {
                ...
                if (countdownRef.current == null) {
                    const expiresIn =
                        Number(status.expiresInSeconds) > 0
                            ? Number(status.expiresInSeconds)
                            : DEFAULT_EXPIRES_IN_SECONDS;
                    setSecondsLeft(
                        Math.max(1, expiresIn - QR_EXPIRY_BUFFER_SECONDS),
                    );
    ...
    const handleSyncCatalog = async () => {
        ...
        try {
            ...
            toast.error(
                error?.message || t("profile.juejin.catalogSyncFailed"),
            );
        } finally {
            setBusy(false);
        }
    ...
    return (
        <div className="mt-4 rounded-xl border border-border bg-surface p-5">
            ...
            {!loading && expired && (
                <div className="mt-4 text-caption font-semibold text-warning">
                    {t("profile.juejin.sessionExpired")}
                </div>
            )}
            ...
            {!loading && !authorized && state !== "UNAVAILABLE" && (
                ...
                >
                    ...
                    {expired
                        ? t("profile.juejin.reauthorize")
                        : t("profile.juejin.connect")}
                </button>
            )}
            ...
            {!loading && state === "UNAVAILABLE" && (
                <div className="mt-4 text-caption text-warning">
                    {t("profile.juejin.unavailable")}
                </div>
            )}
                ...
                {state === "QUEUED" ? (
                    <div className="flex min-h-56 flex-col items-center justify-center gap-3 py-6 text-center">
                        ...
                        <div className="text-sm font-bold text-ink-secondary">
                            {t("profile.juejin.queued")}
                        </div>
                        {queuePosition != null && (
                            <div className="text-caption font-medium text-ink-muted">
                                {t("profile.juejin.queuePosition", {
                        ...
                        {estimatedWait != null && (
                            <div className="text-caption font-medium text-ink-muted">
                                {t("profile.juejin.estimatedWait", {
                                    wait: Math.max(0, estimatedWait),
                                })}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-4 py-2 text-center">
                        ...
                        <div className="flex items-center gap-2 text-caption font-bold text-accent">
                            ...
                            <span>
                                {secondsLeft != null
                                    ? `${t("profile.juejin.waiting")} ${secondsLeft}s`
                                    : t("profile.juejin.waiting")}
                            </span>
                        </div>
                    </div>
                )}
