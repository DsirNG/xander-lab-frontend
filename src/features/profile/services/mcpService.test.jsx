...
describe("csdnService", () => {
    it("授权流程端点映射", () => {
        ...
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/publishing/csdn/authorization/start",
        );
        csdnService.getAuthorizationStatus();
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/publishing/csdn/authorization/status",
            undefined,
            { _silent: true, dedupe: false },
        );
        csdnService.cancelAuthorization();
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/publishing/csdn/authorization/cancel",
            undefined,
            { _silent: true, dedupe: false },
        );
        csdnService.disconnect();
        expect(apiMock["delete"]).toHaveBeenCalledWith(
            "/api/publishing/csdn/authorization",
        );
    });
});
...
describe("mcpOAuthService", () => {
    it("授权请求查询/审批/拒绝端点映射", () => {
        ...
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/mcp/oauth/authorize/requests/abc%2F1",
            undefined,
            { _silent: true, dedupe: false },
        );
        ...
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/mcp/oauth/authorize/requests/req-1/deny",
        );
    });
