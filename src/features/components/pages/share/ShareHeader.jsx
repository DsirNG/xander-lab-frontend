    ...
    return (
        <header className="h-16 flex-shrink-0 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 z-50 shadow-sm">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                <button onClick={onTourStart} className="px-3 sm:px-5 py-2 sm:py-3 text-slate-400 hover:text-primary hover:bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-black transition-all active:scale-95 flex items-center gap-2" title="重启新手向导">
                    <Compass className="w-4 h-4" /> <span className="hidden sm:inline">新手向导</span>
                </button>
                <button onClick={onPublish} className="px-4 sm:px-10 py-2 sm:py-3 bg-primary hover:bg-primary-600 text-white rounded-2xl text-[10px] font-black shadow-xl shadow-primary/20 transition-all active:scale-95 flex items-center gap-2">
                    <Share2 className="w-4 h-4" /> <span className="hidden sm:inline">发布至组件库</span>
                </button>
            </div>
        </header>
    );
