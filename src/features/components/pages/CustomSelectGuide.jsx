...
const DependencyCard = ({ title, items }) => (
    <div className="p-6 rounded-2xl bg-white  border border-slate-200 ">
        <div className="text-sm uppercase tracking-wider text-slate-500 font-bold mb-4">
            {title}
        </div>
        <div className="flex flex-wrap gap-2">
            {items.map((item, i) => (
                <span
                    key={i}
                    className="px-3 py-1 rounded-full bg-slate-100  text-slate-700  text-xs font-mono border border-slate-200"
                >
                    {item}
                </span>
            ))}
            ...
            <div className="max-w-6xl mx-auto pt-10 px-4 sm:px-6">
                ...
                <div className="mb-12">
                    <div className="text-3xl sm:text-4xl font-black text-slate-900  mb-4 break-words">
                        CustomSelect
                        <span className="ml-4 text-lg font-normal text-slate-500 ">
                            {t("common.componentSource")}
                        </span>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    ...
                    <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
                        ...
                        <ul className="space-y-2">
                            {t("components.customSelect.featureList", {
                                returnObjects: true,
                            })?.map((feat, i) => (
                                <li
                                    key={i}
                    ...
                    <div>
                        <div className="flex items-end justify-between mb-4 px-2 flex-wrap gap-2">
                            <div>
                                ...
                                <div className="text-slate-500  text-sm">
                                    {t("common.styleLayerDesc")}
                                </div>
                            </div>
                            <span className="text-xs font-mono text-slate-400 break-all">
                                src/.../CustomSelect/index.module.css
                            </span>
                        </div>
