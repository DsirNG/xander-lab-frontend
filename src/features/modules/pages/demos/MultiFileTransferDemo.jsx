...
const getFileIconSVG = (type) => {
    if (type === "pdf")
        return `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f43f5e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>`;
...
const MultiFileTransferDemo = () => {
    ...
    const dragDrop = useDragDrop({
        getDragPreview: (item) => {
            ...
            const dragItems = isDraggingSelected
                ...
                <div className="lg:col-span-7 bg-white  border border-slate-200  rounded-[2.5rem] overflow-hidden shadow-2xl">
                            ...
                            {files.map((file) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        onPointerDown={(e) =>
                                            dragDrop.handleDragStart(file, e)
                                        }
                                    ...
                                    >
                                        <div
                                            className={`mr-4 w-12 h-12 flex items-center justify-center rounded-xl transition-all duration-300 ${
                                                isSelected
                                        ...
                                        <div className="flex-grow min-w-0">
                                            <div
                                                className={`text-sm font-bold truncate ${isSelected ? "text-primary" : "text-slate-600 "}`}
                                            >
                                                {file.name}
                                            </div>
                                        ...
                                        <GripVertical
                                            className={`ml-4 w-4 h-4 transition-colors ${isSelected ? "text-primary-300" : "text-slate-200 group-hover:text-slate-400"}`}
                                        />
                                    </motion.div>
                                );
                            })}
                    ...
                    <div className="p-6 bg-slate-50 /30 border-t border-slate-100 ">
                        <div className="flex items-center space-x-3 text-xs font-bold text-slate-400">
                            ...
                            <span>
                                {selectedIds.size} ITEMS READY FOR TRANSFER
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-5 space-y-6">
                    {folders.map((folder) => (
                        <div
                            key={folder.id}
                            onPointerMove={(e) =>
                                dragDrop.handleDragOver(folder, e)
                            }
                            onPointerLeave={dragDrop.handleDragLeave}
                            onPointerUp={(e) => dragDrop.handleDrop(folder, e)}
                            className={`p-8 rounded-[2.5rem] border-2 border-dashed transition-all duration-500 relative min-h-[220px] group ${
                                ...
                                <div className="flex items-start justify-between mb-6">
                                    <div
                                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                                            dragDrop.dragOverItem?.id ===
                                            folder.id
                                                ? "bg-primary text-white rotate-12 scale-110 shadow-xl"
                                                : "bg-slate-100  text-slate-400 group-hover:-rotate-3"
                                        }`}
                                    >
                                        {dragDrop.dragOverItem?.id ===
                                        folder.id ? (
                            ...
                            {dragDrop.dragOverItem?.id === folder.id && (
                                ...
                                >
                                    <div className="w-full h-full rounded-[2rem] bg-primary/10 backdrop-blur-sm border-2 border-primary border-dashed flex flex-col items-center justify-center">
                                        ...
                                        <div className="text-primary text-sm font-black tracking-widest uppercase">
                                            Release to transfer
                                        </div>
                                    </div>
                                </motion.div>
