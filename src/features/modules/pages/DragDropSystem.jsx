            ...
            <div className="max-w-4xl mx-auto pt-10 sm:pt-12 px-4 sm:px-6">
                ...
                <div className="ml-4 mb-16 sm:mb-24">
                    {phases.map((phase, index) => (
                        <PhaseCard key={index} phase={phase} index={index} color="blue" />
                    ))}
                </div>
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {t('modules.dragdrop.files', { returnObjects: true }).map((file, i) => (
                                    <div key={i} className="flex items-center space-x-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-colors min-w-0">
                                        <div className="p-2 bg-blue-500/20 rounded-lg flex-shrink-0">
                                            <Box aria-hidden="true" className="w-5 h-5 text-blue-400" />
                                        </div>
                                        <div className="min-w-0">
                                            <div className="text-white font-mono text-caption sm:text-sm break-all">{file.name}</div>
                                            <div className="text-slate-400 text-xs mt-0.5">{file.role}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
