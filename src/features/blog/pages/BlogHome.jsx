...
const BlogHome = () => {
    ...
    const fetchMoreBlogs = useCallback(async () => {
        ...
        try {
            ...
            if (data && data.records) {
                ...
                setHasMore(
                    getHasMore(data, (nextPage - 1) * 10 + data.records.length),
                );
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="text-xs text-ink-muted">
                            {loading
                                ? t("blog.loading")
                                : t("blog.foundArticles", { count: total })}
                        </div>
                        {loadingMore && (
            ...
            {loading && page === 1 ? (
                <div
                    className={`grid ${viewMode === "grid" ? "grid-cols-1 md:grid-cols-2 xl:grid-cols-3" : "grid-cols-1"} gap-4 animate-pulse`}
                >
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            key={i}
