...
const getTopicStyle = (topic = "") => {
    if (topic.includes("设计") || topic.includes("API"))
        return { icon: Link2, color: "text-blue-500", bg: "bg-blue-50" };
    if (
    ...
    const columns = [
        {
            ...
            render: (plan) => {
                ...
                const mainTag =
                    plan.topics?.[0]?.topic ||
                    (plan.topic.includes(t("blogPlans.keys.design", "设计"))
                        ? t("blogPlans.tags.techPractice", "技术实践")
                        : t("blogPlans.tags.contentGen", "内容生成"));
                const secondaryTag = plan.runOnce
                    ? t("blogPlans.tags.manualWrite", "手动编写")
                    : t("blogPlans.tags.autoGenerate", "自动生成");
                ...
                return (
                    <div className="flex items-start gap-4 py-1">
                        ...
                        <div className="min-w-0 flex-1">
                            <div
                                className="truncate text-sm font-bold text-ink mb-1.5"
                                title={plan.topic}
                            >
                                {plan.topic}
                            </div>
                            <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
                                <span
                                    className={`px-2 py-0.5 rounded-full ${getTagStyle(mainTag)}`}
                                >
                                    {mainTag}
                                </span>
                                <span
                                    className={`text-ink-muted ${getTagStyle(secondaryTag)}`}
                                >
                                    {secondaryTag}
                                </span>
                            </div>
            ...
            render: (plan) => (
                <div className="min-w-0">
                    <span
                        ...
                        title={plan.nextRunAt}
                    >
                        {plan.nextRunAt
                            ? new Date(plan.nextRunAt).toLocaleDateString()
                            : "—"}
                    </span>
                    <span className="mt-1 block truncate text-xs text-ink-muted">
                        {plan.nextRunAt
                            ? t("blogPlans.tomorrow", "(明天)")
                            : ""}
                    </span>
                </div>
            ),
                ...
                <div className="relative flex shrink-0 flex-col sm:flex-row items-start sm:items-center justify-between gap-5 overflow-hidden rounded-[24px] bg-gradient-to-r from-[#F4F6FE] to-[#F8F4FD] p-6 sm:px-8 sm:py-7">
                    <div className="relative z-10 min-w-0">
                        <h1 className="text-[22px] font-bold text-ink">
                            {t("blogPlans.title", "发布计划")}
                        </h1>
                        <p className="mt-1.5 text-sm text-ink-muted">
                            {t(
                                "blogPlans.subtitle",
                                "设置主题与发布时间，自动生成并定时发布文章，让内容创作持续发生。",
                            )}
                        </p>
                    </div>
                    <div className="relative z-10 flex shrink-0 items-center gap-3">
                        ...
                        >
                            ...
                            <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                                {t("blogPlans.aiGenerate", "AI 生成计划")}
                            </span>
                        </button>
                        <button
                            onClick={openCreate}
