...
export const blogPlanService = {
    ...
    updatePlan: (id, payload, config) =>
        patch(`/api/blog-plans/${id}`, payload, config),
    updatePlanStatus: (id, action, config) =>
        patch(`/api/blog-plans/${id}/status`, { action }, config),
    deletePlan: (id, config) => del(`/api/blog-plans/${id}`, undefined, config),
    /** 手动立即执行一次（走同一条生成-审核-发布管线） */
    triggerPlan: (id, config) =>
        post(`/api/blog-plans/${id}/trigger`, undefined, config),
    ...
    getUnreadCount: (config) =>
        get("/api/notifications/unread-count", undefined, config),
