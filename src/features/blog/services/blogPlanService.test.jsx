...
const { blogPlanService, PLAN_STATUS, RUN_STATUS } =
    await import("./blogPlanService.js");
...
describe("blogPlanService 运行记录与 AI 生成", () => {
    it("运行列表/详情/下一次运行时间", () => {
        ...
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/blog-plans/7/runs",
            { page: 3, size: 10 },
            undefined,
        );
        blogPlanService.getRun(7, 99);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/blog-plans/7/runs/99",
            undefined,
            undefined,
        );
        blogPlanService.nextRun(7);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/api/blog-plans/7/next",
            { count: 1 },
            undefined,
        );
    ...
    it("主题细化与 AI 生成计划", () => {
        ...
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/blog-plans/topics/generate",
            { seed: "s" },
            undefined,
        );
        blogPlanService.aiGenerate({ days: 7 });
        expect(apiMock.post).toHaveBeenCalledWith(
            "/api/blog-plans/ai-generate",
            { days: 7 },
            undefined,
        );
    });
