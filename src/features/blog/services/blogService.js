...
\`\`\`javascript
function Counter() {
  // 每次调用 useState，React 会在 Fiber 的链表上创建一个 Hook 节点
...
因为 React 依赖 Hooks 调用的**顺序**来确定哪个 state 对应哪个 \`useState\`。如果在循环、条件或嵌套函数中调用 Hooks，可能会导致状态错乱。

\`\`\`javascript
// ❌ 错误示范 - 不要在条件语句中调用 Hook
...
2. **只在 React 函数中调用 Hooks** — 自定义 Hook 或函数组件
3. **使用 ESLint 插件** — \`eslint-plugin-react-hooks\` 强制执行规则
4. **合理拆分 state** — 相关联的状态放一起，不相关的分开
5. **善用自定义 Hook** — 抽取可复用的逻辑

---

*掌握 Hooks 原理不仅能帮助你写出更高效的代码，还能在排查问题时快速定位根因。*`,
    tags: ['React', 'Hooks', '原理'],
    category: 'frontend',
    author: 'Xander',
    date: '2026-02-08',
