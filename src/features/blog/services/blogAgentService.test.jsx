...
describe('blogAgentService 任务 CRUD', () => {
  it('创建/会话列表/详情端点映射', () => {
    ...
    expect(apiMock.post).toHaveBeenCalledWith('/api/blog-agent/tasks', { brief: 'b' }, { signal: 1 })
    blogAgentService.getSessions()
    expect(apiMock.get).toHaveBeenCalledWith('/api/blog-agent/tasks', undefined, undefined)
    blogAgentService.getTask(9)
    expect(apiMock.get).toHaveBeenCalledWith('/api/blog-agent/tasks/9', undefined, undefined)
  })
})
...
describe('blogAgentService 流式接口', () => {
  it('subscribeTaskEvents 无 afterEventId 时不注入参数与头', () => {
    ...
    expect(apiMock.getStream).toHaveBeenCalledWith('/api/blog-agent/tasks/9/events', {
      signal: 1,
      onEvent,
      params: undefined,
      headers: undefined,
  ...
  it('runTaskStream 与 reviseTaskStream 透传 onEvent', () => {
    ...
    expect(apiMock.postStream).toHaveBeenCalledWith('/api/blog-agent/tasks/9/run/stream', undefined, {
      onEvent, signal: 1,
    })
    blogAgentService.reviseTaskStream(9, '新内容', onEvent)
    expect(apiMock.postStream).toHaveBeenCalledWith('/api/blog-agent/tasks/9/messages/stream', { content: '新内容' }, { onEvent })
  })
})
...
describe('blogAgentService 同步执行', () => {
  it('runTask 关闭幂等去重并放宽超时', () => {
    ...
    expect(apiMock.post).toHaveBeenCalledWith(
      '/api/blog-agent/tasks/9/run', undefined,
  ...
  it('publishTask 关闭幂等去重且不限时', () => {
    ...
    expect(apiMock.post).toHaveBeenCalledWith(
      '/api/blog-agent/tasks/9/publish', undefined,
