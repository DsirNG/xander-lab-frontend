...
export const blogAgentService = {
    ...
    subscribeTaskEvents: (id, afterEventId, onEvent, config) =>
    ...
    runTask: (id, config) =>
        post(`${BASE}/${id}/run`, undefined, {
            dedupe: false,
            timeout: 120000,
            ...config,
        }),
    runTaskStream: (id, onEvent, config) =>
        postStream(`${BASE}/${id}/run/stream`, undefined, {
            onEvent,
            ...config,
        }),
    reviseTaskStream: (id, content, onEvent, config) =>
        postStream(
            `${BASE}/${id}/messages/stream`,
            { content },
            { onEvent, ...config },
        ),
    publishTask: (id, config) =>
        post(`${BASE}/${id}/publish`, undefined, {
            dedupe: false,
            timeout: 0,
            ...config,
        }),
