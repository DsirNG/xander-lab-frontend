...
export const NotificationProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            const armWatchdog = () => {
                ...
                watchdogTimer = setTimeout(() => {
                    console.warn(
                        "[SSE] watchdog: no data for 90s, forcing reconnect",
                    );
                    ctrl.abort();
                }, 90000);
            };
            ...
            blogPlanService
                .subscribeNotifications(
                    ...
                    },
                    {
                        ...
                        _silent: true,
                        onProgress: () => {
                            if (isCurrentConnection()) armWatchdog();
                        },
                    },
                )
                .then(() => {
                ...
                .catch((err) => {
                    ...
                    const aborted =
                        err?.code === "ERR_CANCELED" || err?.isCancelled;
                    if (aborted) {
                        setConnected(false);
                        console.warn(
                            "[SSE] stream cancelled, scheduling reconnect",
                        );
                        scheduleReconnect();
                        return;
                    }
        ...
        const onTokenRefresh = () => {
            if (alive && isLoggedInRef.current) {
