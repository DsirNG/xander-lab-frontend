 ...
 */
const PublishEditor = forwardRef(
    (
        {
            isPreview,
            onEditMode,
        ...
        const getFormattingRange = () => {
            ...
            if (start !== end)
                return { start, end, selected: true, cursor: start };
        ...
        const applyLineFormat = (prefix) => {
            ...
            const firstLineMarkerLength =
                lines[0].match(blockMarkerPattern)?.[0].length ?? 0;
            const replacement = lines
                .map(
                    (line) =>
                        `${prefix}${line.replace(blockMarkerPattern, "")}`,
                )
                .join("\n");
            ...
            const nextCursor =
                range.start +
                prefix.length +
                Math.max(0, cursorInLine - firstLineMarkerLength);
            replaceRange(range.start, range.end, replacement, {
                start: nextCursor,
                end: nextCursor,
            });
        ...
        const applyCodeFormat = () => {
            ...
            const nextCursor =
                contentStart + Math.max(0, range.cursor - range.start);
            replaceRange(range.start, range.end, replacement, {
                start: nextCursor,
                end: nextCursor,
            });
        ...
        const insertLibraryImage = (image) => {
            ...
            const alt = imageAltFromName(image.originalName)
                .replaceAll("[", "")
                .replaceAll("]", "")
                .replace(/\r?\n/g, " ");
            insertAtSelection(`\n\n![${alt}](${image.url})\n\n`);
            ...
            requestAnimationFrame(() => {
                if (textareaRef.current)
                    textareaRef.current.scrollTop = viewport.editorY;
                window.scrollTo({ top: viewport.windowY, behavior: "auto" });
            });
        };

        const toolItemClass = useCallback(
            (isActive) =>
            ...
            <div className="flex flex-wrap items-center gap-1">
                {formatActions.map(({ key, icon: Icon, prefix, code }) => (
                    <button
                        ...
                        onClick={() =>
                            code ? applyCodeFormat() : applyLineFormat(prefix)
                        }
                ...
                {insertActions.map(
                    ({ key, icon: Icon, markdown, cursorBack, image }) => (
                        <button
                            key={key}
                            type="button"
                            title={t(`blog.editor.${key}`)}
                            ...
                            onClick={() =>
                                image
                                    ? openImageLibrary()
                                    : insertAtSelection(markdown, cursorBack)
                            }
                            className={toolItemClass(false)}
                        >
                            <Icon className="h-4 w-4 shrink-0" />
                ...
                {content ? (
                    ...
                    <div className="flex min-h-[45vh] flex-col items-center justify-center text-ink-faint">
                        ...
                        <div className="text-caption font-semibold">
                            {t("blog.noContent", "No content yet")}
                        </div>
                    </div>
                )}
            </article>
            ...
            <section className="relative flex h-full min-h-0 flex-col bg-canvas">
                ...
                <div className="flex h-9 shrink-0 items-center justify-between border-t border-border bg-surface-muted/50 px-4 text-micro text-ink-faint">
                    <div className="flex items-center gap-5">
                        <span>
                            {t("blog.editor.characters", {
                                count: content.length,
                            })}
                        </span>
                        <span>
                            {t("blog.editor.lines", { count: lineCount })}
                        </span>
                    </div>
            ...
            <>
                <div className="flex h-full min-h-0 flex-col">
                    <div className="flex min-h-16 shrink-0 items-center gap-3 border-b border-border bg-canvas px-4 sm:px-6">
                        ...
                        />
                        {isMobile && (
                            <div className="flex shrink-0 bg-surface-muted p-1 rounded-lg border border-border">
                                <button
                                    onClick={onEditMode}
                        ...
                        {isMobile ? (
                            isPreview ? (
                                <section className="flex h-full min-h-0 flex-col bg-surface">
                                    ...
                                    <div className="flex h-9 shrink-0 items-center justify-end border-t border-border bg-surface-muted/50 px-4 text-micro text-ink-faint">
                                        {t("blog.editor.previewMode")}
                                    </div>
                                </section>
                            ) : (
                                renderEditor()
