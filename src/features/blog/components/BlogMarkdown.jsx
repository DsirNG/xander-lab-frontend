...
const markdownComponents = {
    ...
    blockquote({ children, ...props }) {
        return <blockquote className="my-4 rounded-r-lg border-l-4 border-accent/50 bg-accent/5 py-2 pl-4 text-ink-secondary [&>p]:my-1" {...props}>{children}</blockquote>;
    },
};

const BlogMarkdown = ({ content, className = '' }) => (
    <div className={`prose prose-sm max-w-none sm:prose-base prose-headings:font-bold prose-headings:tracking-tight prose-headings:text-ink prose-p:text-ink-secondary prose-li:text-ink-secondary prose-strong:text-ink prose-h2:mb-4 prose-h2:mt-8 prose-h2:text-xl prose-h3:mb-3 prose-h3:mt-6 prose-h3:text-lg prose-li:leading-relaxed prose-p:leading-relaxed prose-pre:m-0 prose-pre:bg-transparent prose-pre:p-0 ${className}`}>
