...
const PlanAiGenerateModal = ({ isOpen, onClose, onSaved }) => {
    ...
    const submit = async () => {
        ...
        try {
            ...
            toast.success(
                t("blogPlans.generatedPlans", { count: created.length }),
            );
            await onSaved?.();
        } catch (e) {
            toast.error(
                e?.response?.data?.message || t("blogPlans.generateFailed"),
            );
        } finally {
            setGenerating(false);
        }
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <label className="flex items-center gap-2 text-sm text-ink-secondary">
                        <input
                            type="checkbox"
                            checked={form.syncCsdn}
                            onChange={(e) =>
                                setForm({ ...form, syncCsdn: e.target.checked })
                            }
                    ...
                    <label className="flex items-center gap-2 text-sm text-ink-secondary">
                        <input
                            type="checkbox"
                            checked={form.syncJuejin}
                            onChange={(e) =>
                                setForm({
                                    ...form,
                                    syncJuejin: e.target.checked,
                                })
                            }
                            className="h-4 w-4 rounded border-border text-accent focus:ring-accent"
                        />
