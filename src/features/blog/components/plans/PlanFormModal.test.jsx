...
const planServiceMock = vi.hoisted(() => ({ createPlan: vi.fn(), updatePlan: vi.fn() }));
const toastMock = vi.hoisted(() => ({ success: vi.fn(), error: vi.fn() }));

vi.mock('../../services/blogPlanService', () => ({ blogPlanService: planServiceMock }));
...
describe('PlanFormModal business controls', () => {
  it('submits selected schedule, AI direction, date and platforms', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('blogPlans.topicPlaceholder'), {
    ...
    await waitFor(() => expect(planServiceMock.createPlan).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('restores platform selections while editing', () => {
    render(<PlanFormModal isOpen plan={{
      id: 7,
      topic: 'Existing plan',
      scheduleType: 'WEEKLY',
    ...
    expect(screen.getByLabelText('blogPlans.firstExecutionDate')).toHaveValue('2026-09-07');
  });
});
