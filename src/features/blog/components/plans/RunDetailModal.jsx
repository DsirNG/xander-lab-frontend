...
const Row = ({ label, children }) => (
  <div className="flex items-start justify-between gap-6 py-2 text-sm">
    ...
    <span className="break-all text-right text-ink-secondary">{children || '—'}</span>
  </div>
);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={t('blogPlans.runDetailTitle')} width="max-w-lg">
      <div className="divide-y divide-border">
        ...
        <Row label={t('blogPlans.reviewResult')}>
          {run.reviewPass == null ? '—' : (run.reviewPass ? t('blogPlans.reviewPassed') : t('blogPlans.reviewRejected'))}
        </Row>
        {run.reviewPass === false && run.reviewReason && (
          <Row label={t('blogPlans.reviewReason')}>{run.reviewReason}</Row>
        )}
