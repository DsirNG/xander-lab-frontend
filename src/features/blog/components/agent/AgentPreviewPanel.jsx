...
const AgentPreviewPanel = ({
  ...
  if (!task) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-white px-6 text-center">
        <p className="text-sm font-semibold text-slate-500">{t('blog.agent.previewEmpty')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      <header className="flex shrink-0 flex-wrap items-start justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:px-5">
        <div className="min-w-0 flex-1">
          <p className="text-xs font-bold uppercase tracking-widest text-primary">{t('blog.agent.article')}</p>
            ...
            task.status === 'ready' && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-black text-white disabled:opacity-60"
                >
            ...
            <div className="mt-4 space-y-4">
              <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-3 space-y-3">
                  <div>
                    <p className="text-xs font-bold text-slate-500">{t('blog.agent.mustCover')}</p>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {asArray(contentBoundary.mustCover).map((item) => (
                        <span key={item} className="rounded-lg bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">
                          {item}
                        </span>
                      ))}
                    </div>
                  ...
                  {asArray(contentBoundary.relatedExpansion).length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-500">{t('blog.agent.relatedExpansion')}</p>
                  ...
                  {asArray(contentBoundary.outOfScope).length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-500">{t('blog.agent.outOfScope')}</p>
              ...
              {knowledgeGraph.enabled && graphNodes.length > 0 && (
                <section className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
                  ...
                  <p className="mt-2 text-xs leading-5 text-indigo-900/70">{knowledgeGraph.reason}</p>
                  ...
                  <div className="mt-3 space-y-2">
                    {graphEdges.map((edge, index) => (
                      <div
                        ...
                        className="rounded-lg bg-white/80 px-2.5 py-2 text-xs leading-5 text-indigo-950"
                      >
              ...
              {(illustrations.length > 0 || task.illustrationStatus) && (
                <section className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4">
                  ...
                  {task.illustrationError && (
                    <p className="mt-3 text-xs leading-5 text-amber-700">{task.illustrationError}</p>
                  )}
                </section>
              )}
                  ...
                  {taskData.sources?.length ? (
                    taskData.sources.map((source) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="block rounded-xl bg-white p-3 transition hover:bg-primary/5"
                      >
                        <p className="line-clamp-2 text-sm font-bold text-slate-800">{source.title}</p>
                        <p className="mt-1 text-xs text-slate-500">{source.publisher || source.reliability}</p>
                        <ExternalLink className="mt-2 h-3.5 w-3.5 text-primary" />
                      </a>
                    ))
                  ) : (
                    <p className="text-sm leading-6 text-slate-500">{t('blog.agent.noSources')}</p>
                  )}
                </div>
              </section>
