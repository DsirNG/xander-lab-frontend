...
const AgentPreviewPanel = ({
    ...
    const isLatestVersion =
        !selectedVersion ||
        String(selectedVersion.id) === String(versions[0]?.id);
    ...
    const graphLabels = new Map(
        graphNodes.map((node) => [node.id, node.label || node.id]),
    );
    ...
    return (
        <div className="flex h-full min-h-0 flex-col bg-canvas">
            ...
            <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 pb-safe sm:px-6">
                {article.content ? (
                ...
                <div className="mt-8 border-t border-border pt-4">
                    ...
                    >
                        {metaOpen
                            ? t("blog.agent.hideMeta")
                            : t("blog.agent.showMeta")}
                    </button>
                        ...
                        <div className="mt-4 space-y-4">
                                ...
                                <div className="mt-3 space-y-3">
                                    <div>
                                        <div className="text-xs font-bold text-ink-muted">
                                            {t("blog.agent.mustCover")}
                                        </div>
                                        <div className="mt-2 flex flex-wrap gap-1.5">
                                            {asArray(
                                                contentBoundary.mustCover,
                                            ).map((item) => (
                                                <span
                                                    key={item}
                                                    className="rounded-lg bg-accent/10 px-2 py-1 text-xs font-semibold text-accent"
                                                >
                                                    {item}
                                                </span>
                                            ))}
                                    ...
                                    {asArray(contentBoundary.relatedExpansion)
                                        .length > 0 && (
                                        <div>
                                            <div className="text-xs font-bold text-ink-muted">
                                                {t(
                                                    "blog.agent.relatedExpansion",
                                                )}
                                            </div>
                                            <div className="mt-1 text-xs leading-5 text-ink-secondary">
                                                {asArray(
                                                    contentBoundary.relatedExpansion,
                                                ).join("、")}
                                            </div>
                                        </div>
                                    )}
                                    {asArray(contentBoundary.outOfScope)
                                        .length > 0 && (
                                        <div>
                                            <div className="text-xs font-bold text-ink-muted">
                                                {t("blog.agent.outOfScope")}
                                            </div>
                                            <div className="mt-1 text-xs leading-5 text-ink-muted">
                                                {asArray(
                                                    contentBoundary.outOfScope,
                                                ).join("、")}
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="mt-3 space-y-2">
                                            {graphEdges.map((edge, index) => (
                                                ...
                                                >
                                                    ...
                                                    <span className="mx-1.5 text-info/60">
                                                        →
                                                    </span>
                            ...
                            {(illustrations.length > 0 ||
                                ...
                                <section className="rounded-2xl border border-info/20 bg-info-soft/60 p-4">
                                    ...
                                    <div className="mt-2 text-xs leading-5 text-info-fg/70">
                                        {t(
                                            `blog.agent.illustrationStatuses.${task.illustrationStatus || "none"}`,
                                        )}
                                    </div>
                                    {illustrations.length > 0 && (
                                    ...
                                    {task.illustrationError && (
                                        <div className="mt-3 text-xs leading-5 text-warning-fg">
                                            {task.illustrationError}
                                        </div>
                                    )}
                                </section>
