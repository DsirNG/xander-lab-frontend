...
describe('usePlanActions 成功路径', () => {
  it.each([
    ...
    expect(planServiceMock.updatePlanStatus).toHaveBeenCalledWith(9, status)
  ...
  it('trigger 立即执行并提示', async () => {
    ...
    const { result } = renderHook(() => usePlanActions({ onChanged: vi.fn(), t }))
  ...
  it('remove 确认后删除并刷新', async () => {
    ...
    expect(window.confirm).toHaveBeenCalledWith('t:blogPlans.deleteConfirm'.replace('{{topic}}', plan.topic))
...
describe('usePlanActions 失败路径', () => {
  ...
  it('无后端错误信息时回退失败文案', async () => {
    ...
    expect(toastMock.error).toHaveBeenCalledWith('t:blogPlans.triggerFailed')
  })
