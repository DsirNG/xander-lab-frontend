...
vi.mock("../services/blogService", () => ({
    ...
    BLOG_STATUS: { DRAFT: 0, PUBLISHED: 1, TRASH: -1 },
}));
...
describe("usePublishSubmit 表单校验", () => {
    it("缺少必填字段时仅提示警告且不发请求", async () => {
        const { result } = renderHook(() =>
            usePublishSubmitHook({
                ...defaultProps,
                formData: { title: "", content: "内容", categoryId: 1 },
            }),
        );
        await act(async () => {
...
describe("usePublishSubmit 发布流程", () => {
    ...
    it("编辑模式：更新草稿为已发布状态并跳转个人页", async () => {
        ...
        expect(blogServiceMock.updateBlog).toHaveBeenCalledWith(
            7,
...
describe("usePublishSubmit 发布失败降级", () => {
    it("网络状态不确定时轮询发布状态，命中已发布即视为成功", async () => {
        ...
        blogServiceMock.getPublishStatus.mockResolvedValue({
            status: "published",
        });
        const { result } = renderHook(() => usePublishSubmitHook(defaultProps));
