...
const EMPTY_FORM = { title: '', categoryId: '', summary: '', content: '', tags: [] };
...
const usePublishForm = ({ isEditMode }) => {
    ...
    const setDefaultCategory = useCallback((value) => {
        if (!value) return;
        setFormData((current) => ({ ...current, categoryId: current.categoryId || value }));
    }, []);
    ...
    useEffect(() => {
        ...
        const isEmpty = !formData.title && !formData.summary && !formData.content && formData.tags.length === 0;
        if (isEmpty) return;
    ...
    useEffect(() => () => {
        ...
        if (!disabledRef.current && !isEditModeRef.current) {
            const isEmpty = !latestFormRef.current.title
