...
const buildImageReliefGroup = async (sceneSpec, signal) => {
    ...
    const requestedResolution = Math.round(
        readNumber(relief.resolution, 128, 16, LIMITS.reliefResolution),
    );
    let gridWidth =
        sourceAspect >= 1
            ? requestedResolution
    ...
    if (gridWidth * gridHeight > LIMITS.reliefCells) {
        const reduction = Math.sqrt(
            LIMITS.reliefCells / (gridWidth * gridHeight),
        );
    ...
    const textureScale = Math.min(
        1,
        LIMITS.textureEdge / Math.max(image.naturalWidth, image.naturalHeight),
    );
