...
describe("studioService 项目接口", () => {
    ...
    it("文件树/内容/可见性/公开接口端点映射", () => {
        fetchFileTree(7);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/studio-api/projects/7/files",
            {},
            { baseURL: "" },
        );
        fetchFileContent(7, "src/main.js");
        expect(apiMock.get).toHaveBeenCalledWith(
            "/studio-api/projects/7/files/content",
            { path: "src/main.js" },
            { baseURL: "" },
        );
        updateProjectVisibility(7, "public");
        expect(apiMock.patch).toHaveBeenCalledWith(
            "/studio-api/projects/7/visibility",
            { visibility: "public" },
            { baseURL: "" },
        );
        fetchPublicProject(7);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/studio-api/public/projects/7",
            {},
            { baseURL: "" },
        );
        fetchPublicFileTree(7);
        expect(apiMock.get).toHaveBeenCalledWith(
            "/studio-api/public/projects/7/files",
            {},
            { baseURL: "" },
        );
        fetchPublicFileContent(7, "a.txt");
        expect(apiMock.get).toHaveBeenCalledWith(
            "/studio-api/public/projects/7/files/content",
            { path: "a.txt" },
            { baseURL: "" },
        );
    });
    ...
    it("downloadPublicProjectSource 使用指定文件名（缺省回退 projectId）", () => {
        ...
        expect(apiMock.download).toHaveBeenCalledWith(
            "/studio-api/public/projects/7/download",
            {
...
describe("studioService 工具函数", () => {
    it("convertPreviewUrl 原样返回，空值回退空串", () => {
        expect(convertPreviewUrl("http://1.localhost:3010/")).toBe(
            "http://1.localhost:3010/",
        );
