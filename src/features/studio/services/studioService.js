...
export async function uploadVueComponent(
    componentFile,
    demoFile = null,
    externalCss = "",
) {
...
export async function fetchFileContent(projectId, filePath) {
    return get(
        `/studio-api/projects/${projectId}/files/content`,
        { path: filePath },
        STUDIO_CONFIG,
    );
}
...
export async function updateProjectVisibility(projectId, visibility) {
    return patch(
        `/studio-api/projects/${projectId}/visibility`,
        { visibility },
        STUDIO_CONFIG,
    );
}
...
export async function fetchPublicFileTree(projectId) {
    return get(
        `/studio-api/public/projects/${projectId}/files`,
        {},
        STUDIO_CONFIG,
    );
}
...
export async function fetchPublicFileContent(projectId, filePath) {
    return get(
        `/studio-api/public/projects/${projectId}/files/content`,
        { path: filePath },
        STUDIO_CONFIG,
    );
}
...
export function downloadPublicProjectSource(projectId, name) {
    return download(`/studio-api/public/projects/${projectId}/download`, {
        filename: `${name || projectId}-source.zip`,
