...
describe('authService.login', () => {
  it('调用 POST /api/auth/login 并保存令牌、用户信息与登录事件', async () => {
    const res = { accessToken: 'at', refreshToken: 'rt', userInfo: { id: 1, name: 'tester' } }
    const eventSpy = vi.spyOn(window, 'dispatchEvent')
    apiMock.post.mockResolvedValue(res)

    await expect(authService.login({ account: 'a', password: 'p' })).resolves.toBe(res)

    expect(apiMock.post).toHaveBeenCalledWith('/api/auth/login', { account: 'a', password: 'p' })
    expect(apiMock.tokenStorage.setToken).toHaveBeenCalledWith('at', { notify: false })
    expect(apiMock.tokenStorage.setRefreshToken).toHaveBeenCalledWith('rt')
    expect(localStorage.getItem('user_info')).toBe(JSON.stringify(res.userInfo))
    expect(eventSpy).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'auth:login', detail: { user: res.userInfo } })
    )
  })
...
describe('authService.logout', () => {
  it('携带 refresh token 静默登出并清空会话、分发登出事件', async () => {
    ...
    expect(eventSpy).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'auth:logout', detail: { reason: 'logout' } })
    )
  })
...
describe('authService.checkCurrentSession', () => {
  ...
  it('有 token 时以静默、去幂等配置请求 /me 并合并自定义 config', async () => {
    ...
    expect(apiMock.get).toHaveBeenCalledWith(
      '/api/auth/me',
      undefined,
      { _silent: true, dedupe: false, retry: 2 }
    )
