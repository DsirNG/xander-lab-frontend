...
describe('useAppUpdate', () => {
  it('挂载时立即检查版本，版本不一致时触发更新', async () => {
    globalThis.fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve(staleManifest) })

    const { result, unmount } = await mount()
    unmount()

    expect(globalThis.fetch).toHaveBeenCalledWith(expect.stringMatching(/^\/version\.json\?t=\d+$/), {
      cache: 'no-store',
    })
  ...
  it('版本一致或请求失败时静默跳过', async () => {
    globalThis.fetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ version: '1.2.3' }) })
  ...
  it('vite:preloadError 事件触发更新', async () => {
    ...
    let unmount
    await mount().then((r) => { unmount = r.unmount })
