...
const AdminUsersPage = () => {
  ...
  const loadUsers = useCallback(async () => {
    try {
      setLoading(true);
      const data = await adminService.listUsers({ keyword: keyword || undefined, page, size: pageSize });
  ...
  const handleToggleStatus = async (user) => {
    ...
    try {
      ...
      toast.success(nextStatus === 0 ? t('admin.users.banned') : t('admin.users.unbanned'));
      await loadUsers();
    } catch {
      // HTTP 层已统一提示
    } finally {
  ...
  const columns = [
    {
      ...
      render: (user) => (
        <div className="min-w-0">
          <div className="truncate text-xs font-bold text-ink">{user.username}</div>
          {user.nickname && (
            <div className="mt-0.5 truncate text-micro text-ink-faint">{user.nickname}</div>
          )}
        </div>
      ),
    },
    {
      ...
      width: '24%',
      render: (user) => (
        <span className="block truncate text-xs font-medium text-ink-muted" title={user.email}>
          {user.email || '—'}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (user) => {
        ...
        return (
          <span
            className={`inline-flex rounded-full px-2 py-0.5 text-micro font-medium ${
              banned ? 'bg-danger-soft text-danger-fg' : 'bg-success-soft text-success-fg'
            }`}
          >
        ...
        const items = [
          {
            ...
            label: banned ? t('admin.users.unban') : t('admin.users.ban'),
            icon: banned ? RotateCcw : Ban,
            danger: !banned,
            disabled: busyId === user.id,
            loading: busyId === user.id,
            loadingLabel: banned ? t('admin.users.unban') : t('admin.users.ban'),
    ...
    <div className="mx-auto flex h-full min-h-0 w-full max-w-5xl flex-col overflow-hidden p-4 sm:p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-ink-muted">{t('admin.users.subtitle')}</p>
          </div>
      ...
      <ConfirmModal
        ...
        title={pendingTarget?.status === 1 ? t('admin.users.banTitle') : t('admin.users.unbanTitle')}
        message={pendingTarget?.status === 1
          ? t('admin.users.banMessage', { name: pendingTarget?.username })
          : t('admin.users.unbanMessage', { name: pendingTarget?.username })}
        confirmText={pendingTarget?.status === 1 ? t('admin.users.ban') : t('admin.users.unban')}
