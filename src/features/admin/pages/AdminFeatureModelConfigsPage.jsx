    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const ordered = FEATURE_KEYS.map(
                (key) =>
                    configData?.find((c) => c.featureKey === key) || {
                        featureKey: key,
                        enabled: false,
                        primaryProviderId: null,
                        primaryModel: null,
                        primaryApiStyle: null,
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (config) => (
                ...
                >
                    {config.enabled
                        ? t("admin.configs.enabledOn")
                        : t("admin.configs.enabledOff")}
                </span>
            ),
        },
        {
    ...
    return (
        <div className="mx-auto flex h-full min-h-0 w-full max-w-5xl flex-col overflow-hidden p-4 sm:p-6">
            ...
            <FeatureConfigModal
                ...
                onSaved={() => {
                    setSavingId(editingConfig?.featureKey);
                    loadData().finally(() => setSavingId(null));
                }}
            />
        </div>
    );
