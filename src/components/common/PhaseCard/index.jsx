...
const PhaseCard = ({ phase, index, color = "primary" }) => {
    ...
    return (
        ...
        >
            <div
                className={`absolute left-[-16px] top-0 w-8 h-8 rounded-full bg-white border-2 ${scheme.border} flex items-center justify-center ${scheme.text} shadow-sm`}
            >
            ...
            <div
                className={`bg-white/50 rounded-3xl p-5 sm:p-8 border border-slate-200 ${scheme.hoverBorder} transition-colors shadow-sm`}
            >
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {phase.points.map((point, i) => (
                        <li
                            key={i}
                            className="flex items-center space-x-2 text-slate-500 text-body"
                        >
