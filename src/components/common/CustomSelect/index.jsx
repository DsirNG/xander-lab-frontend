...
const CustomSelect = ({
    ...
    const checkBoundary = useCallback(() => {
        ...
        if (overflowsBelow) {
            ...
            shouldUpward =
                spaceAbove >= dropdownHeight + 8 || spaceAbove > spaceBelow;
        }

        setIsUpward(shouldUpward);
        const availableSpace = shouldUpward ? spaceAbove : spaceBelow;
        setDropdownMaxHeight(
            Math.max(optionHeightPx + 8, Math.min(240, availableSpace - 8)),
        );
    ...
    const getAlignClass = (alignment) => {
        const validAlign = ["left", "center", "right"].includes(alignment)
            ? alignment
            : "left";
        return styles[
            `align${validAlign.charAt(0).toUpperCase() + validAlign.slice(1)}`
        ];
    };
