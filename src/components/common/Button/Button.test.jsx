...
describe('Button', () => {
  ...
  it('defaults to primary variant with accent background', () => {
    ...
    expect(screen.getByRole('button', { name: '主操作' })).toHaveClass('bg-accent');
  });

  it('applies size proportions', () => {
    const { rerender } = render(<Button size="sm">小</Button>);
    expect(screen.getByRole('button', { name: '小' })).toHaveClass('h-9', 'px-3.5', 'rounded-lg');
    rerender(<Button size="md">中</Button>);
    expect(screen.getByRole('button', { name: '中' })).toHaveClass('h-10', 'px-4', 'rounded-xl');
    rerender(<Button size="lg">大</Button>);
    expect(screen.getByRole('button', { name: '大' })).toHaveClass('h-11', 'px-5', 'rounded-xl');
  });
  ...
  it('replaces the icon with a spinner while loading', () => {
    render(<Button icon={Check} loading>完成</Button>);
    const btn = screen.getByRole('button', { name: '完成' });
    expect(btn.querySelector('.animate-spin')).not.toBeNull();
  ...
  it('renders as a block button with w-full', () => {
    ...
    expect(screen.getByRole('button', { name: '通栏' })).toHaveClass('w-full');
  });
  ...
  it('spreads extra props like aria-label and title', () => {
    render(<Button aria-label="自定义标签" title="提示">文字</Button>);
    expect(screen.getByRole('button', { name: '自定义标签' })).toHaveAttribute('title', '提示');
  });
