 ...
 */
const SIZE_CLASSES = {
    xs: "h-8 px-3 gap-1.5 text-caption rounded-lg",
    sm: "h-9 px-3.5 gap-1.5 text-caption rounded-lg",
    md: "h-10 px-4 gap-2 text-sm rounded-xl",
    lg: "h-11 px-5 gap-2 text-sm rounded-xl",
};
...
const VARIANT_CLASSES = {
    primary:
        "bg-accent text-white shadow-sm hover:bg-accent-700 focus-visible:ring-accent-200",
    ...
    dangerOutline:
        "border border-danger/40 bg-canvas text-danger hover:bg-danger-soft focus-visible:ring-danger/30",
...
const Button = forwardRef(
    (
        ...
        const renderIcon = () =>
            loading ? (
                <Loader2
                    className={`${ICON_CLASSES[size]} shrink-0 animate-spin`}
                    aria-hidden="true"
                />
            ) : (
                <Icon
                    className={`${ICON_CLASSES[size]} shrink-0`}
                    aria-hidden="true"
                />
            );
        ...
        return (
            <button
                ...
                {...rest}
            >
                {(!Icon || iconPosition === "left") && (Icon || loading)
                    ? renderIcon()
                    : null}
                {children}
