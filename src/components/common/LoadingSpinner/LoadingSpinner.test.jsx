...
describe('LoadingSpinner', () => {
  ...
  it('sizes the spinner with the sm size class', () => {
    const { container } = render(<LoadingSpinner fullScreen={false} size="sm" />);
    expect(container.querySelector('.animate-spin')).toHaveClass('w-5', 'h-5');
  });

  it('sizes the spinner with the lg size class', () => {
    const { container } = render(<LoadingSpinner fullScreen={false} size="lg" />);
    expect(container.querySelector('.animate-spin')).toHaveClass('w-12', 'h-12');
  });
})