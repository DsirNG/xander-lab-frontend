...
| `HtmlSandboxPreview` | `@components/common/HtmlSandboxPreview` | 单文件 HTML / SVG 片段的沙箱预览 | `code`、`language`、`minHeight`（默认 280）、`maxHeight`（默认 1200）、`title`；srcdoc + `sandbox="allow-scripts"`（无 same-origin）+ 断网 CSP + pushState shim，内容高度自适应汇报，超出 `maxHeight` 内部滚动。 |
